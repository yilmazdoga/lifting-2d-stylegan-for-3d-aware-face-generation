...
class LiftedGAN(object):
    ...
    def initialize(self, config, training=True):
        ...
        self.PerceptualLoss = PerceptualLoss(requires_grad=False,
                                             loss_type=self.config.perc_type, n_scale=self.config.n_scale,
                                             slice_indices=self.config.vgg_indices).cuda()
        ...
        if training:
            ...
            self.optimizer = torch.optim.Adam(optimize_parameters, lr=config.learning_rate, betas=(0.9, 0.999),
                                              weight_decay=5e-4)
    ...
    def estimate_w_style_v2(self, content_latent, style_latent):
        b = content_latent.shape[0]
        size = self.config.image_size
        ...
        ## predict canonical albedo
        _, style_feat = self.generator(style_latent, input_is_latent=True, randomize_noise=False, return_feat=True)
    ...
    def render(self, canon_depth, canon_albedo, canon_light, view, use_light=True, trans_map=None):
        ...
        recon_im_mask = (
                    recon_depth.detach() < self.max_depth + margin).float()  # invalid border pixels have been clamped at max_depth+margin
    ...
    def train_step(self):
        ...
        summary['image']['input/input_im'] = make_grid(input_im[:b0].clamp(min=-1, max=1) * 0.5 + 0.5, nrow=nrow,
                                                       normalize=False)

        ### Estimation
        canon_depth, canon_albedo, canon_light, view, neutral_style, trans_map, canon_im_raw = self.estimate(styles)
        summary['image']['depth/canon_depth'] = make_grid(self.depth_inv_rescaler(canon_depth[:, None])[:b0], nrow=nrow,
                                                          normalize=True)
        ...
        summary['image']['depth/canon_normal'] = make_grid(canon_normal[:b0].permute(0, 3, 1, 2), nrow=nrow,
                                                           normalize=True)
        ...
        _, perturbed_light_original, perturbed_view_original, perturb_angles = self.perturb(neutral_style, canon_light,
                                                                                            view)
        ...
        summary['image']['perturb/perturbed_im'] = make_grid(perturbed_im[:b0].clamp(min=-1, max=1) * 0.5 + 0.5,
        ...
        if self.config.lam_perturb_im > 0:
            with torch.no_grad():
                ...
                summary['image']['perturb/regenerated_im'] = make_grid(
                    regenerated_im[:b0].clamp(min=-1, max=1) * 0.5 + 0.5, nrow=nrow, normalize=False)
        ...
        if self.config.lam_albedo_rank > 0:
            ...
            if self.config.rank_downsample:
                albedo_residual = F.avg_pool2d(albedo_residual, self.config.rank_downsample,
                                               stride=self.config.rank_downsample, padding=0)
        ...
        if self.config.lam_perturb_im > 0:
            with torch.no_grad():
                perturbed_im_original = \
                self.render(canon_depth, canon_albedo, perturbed_light_original, perturbed_view_original,
                            trans_map=trans_map)[0]
                summary['image']['perturb/perturbed_im_original'] = make_grid(
                    perturbed_im_original[:b0].clamp(min=-1, max=1) * 0.5 + 0.5, nrow=nrow, normalize=False)

            perturbed_styles = self.netSC(styles.detach(), perturbed_light_original, perturbed_view_original)
            regenerated_im = self.generator(perturbed_styles, input_is_latent=True, randomize_noise=False)
            loss_perturb_G = self.image_loss(perturbed_im_original.detach(), regenerated_im,
                                             lam_perc=self.config.lam_perc, mask=None)
    ...
    def add_videos(self, summary):
        ...
        with torch.no_grad():
            ...
            canon_normal_rotate = self.renderer.render_yaw(self.canon_normal[:b0].permute(0, 3, 1, 2),
                                                           ...
                                                           maxr=90).detach().cpu() / 2. + 0.5  # (B,T,C,H,W)
            neutral_shading_rotate = self.renderer.render_yaw(neutral_shading[:b0].permute(0, 3, 1, 2),
                                                              self.canon_depth[:b0], v_before=v0,
                                                              maxr=90).detach().cpu() / 2. + 0.5  # (B,T,C,H,W)

            canon_im_rotate_grid = [make_grid(img, nrow=nrow) for img in
                                    torch.unbind(canon_im_rotate, 1)]  # [(C,H,W)]*T
            canon_im_rotate_grid = torch.stack(canon_im_rotate_grid, 0).unsqueeze(0)  # (1,T,C,H,W)
            canon_normal_rotate_grid = [make_grid(img, nrow=nrow) for img in
            ...
            neutral_shading_rotate_grid = [make_grid(img, nrow=nrow) for img in
                                           torch.unbind(neutral_shading_rotate, 1)]  # [(C,H,W)]*T
    ...
    def test(self, input_code, render=False, recon_normal=False, generate=False, truncation=1.0):
        ...
        with torch.no_grad():
            ...
            canon_depth, canon_albedo, canon_light, view, neutral_style, trans_map, canon_im_raw = self.estimate(
                input_style)
